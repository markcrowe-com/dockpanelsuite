...
{
    internal class SolarizedDarkAutoHideStrip : AutoHideStripBase
    {
        private class TabSolarizedDark : Tab
        {
            internal TabSolarizedDark(IDockContent content)
                : base(content)
            {
            }
        ...
        public SolarizedDarkAutoHideStrip(DockPanel panel)
            : base(panel)
        {
        ...
        private void DrawTabStrip(Graphics g, DockState dockState)
        {
            ...
            {
                foreach (TabSolarizedDark tab in pane.AutoHideTabs)
                    DrawTab(g, tab);
            }
        ...
        private GraphicsPath GetTabOutline(TabSolarizedDark tab, bool transformed, bool rtlTransform)
        {
        ...
        private void DrawTab(Graphics g, TabSolarizedDark tab)
        {
        ...
        private Rectangle GetTabRectangle(TabSolarizedDark tab)
        {
            return GetTabRectangle(tab, false);
        }

        private Rectangle GetTabRectangle(TabSolarizedDark tab, bool transformed)
        {
            DockState dockState = tab.Content.DockHandler.DockState;
            Rectangle rectTabStrip = GetLogicalTabStripRectangle(dockState);
        ...
        {
            GraphicsPath path = GetTabOutline((TabSolarizedDark)tab, true, true);
        ...
        private TabSolarizedDark lastSelectedTab = null;

        protected override void OnMouseHover(EventArgs e)
        {
            var tab = (TabSolarizedDark)TabHitTest(PointToClient(MousePosition));
            if (tab != null)
            {
