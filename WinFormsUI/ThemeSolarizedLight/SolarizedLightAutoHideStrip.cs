...
{
    internal class SolarizedLightAutoHideStrip : AutoHideStripBase
    {
        private class TabSolarizedLight : Tab
        {
            internal TabSolarizedLight(IDockContent content)
                : base(content)
            {
            }
        ...
        public SolarizedLightAutoHideStrip(DockPanel panel)
            : base(panel)
        {
        ...
        private void DrawTabStrip(Graphics g, DockState dockState)
        {
            ...
            {
                foreach (TabSolarizedLight tab in pane.AutoHideTabs)
                    DrawTab(g, tab);
            }
        ...
        private GraphicsPath GetTabOutline(TabSolarizedLight tab, bool transformed, bool rtlTransform)
        {
        ...
        private void DrawTab(Graphics g, TabSolarizedLight tab)
        {
        ...
        private Rectangle GetTabRectangle(TabSolarizedLight tab)
        {
            return GetTabRectangle(tab, false);
        }

        private Rectangle GetTabRectangle(TabSolarizedLight tab, bool transformed)
        {
            DockState dockState = tab.Content.DockHandler.DockState;
            Rectangle rectTabStrip = GetLogicalTabStripRectangle(dockState);
        ...
        {
            GraphicsPath path = GetTabOutline((TabSolarizedLight)tab, true, true);
        ...
        private TabSolarizedLight lastSelectedTab = null;

        protected override void OnMouseHover(EventArgs e)
        {
            var tab = (TabSolarizedLight)TabHitTest(PointToClient(MousePosition));
            if (tab != null)
            {
